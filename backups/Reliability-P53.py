...
def return_test_number():
    ...
    alt_list = [vintage, qty, alt_number, sub_date, product, test, doe_purpose, status]
...
if __name__ == "__main__":
    ...
    if ws.range('B3').value == 'Pre SA' or ws.range('B3').value == 'Post SA':
    ...
    elif ws.range('B3').value == 'Carrier Tests':

        print('Submitting Carrier Test:')
        for i in range(1, 39):
            if ws[i, 14].value == 'Y':
                qty = int(ws[i, 15].value)
                testshort = ws[i, 16].value
                testcase = ws[i, 16].value + ' ' + ws[i, 17].value
                subcat = ws.range('B4').value
                doe_purpose = purdetails
